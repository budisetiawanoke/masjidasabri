...
export default async function DonasiLaporanDetailPage({
  params,
  searchParams,
}: {
  ...
  const report = await loadDetailReport("donasi", campaignId, year, month);
  if (!report) notFound();
  ...
  return (
    <>
      <div className="mx-auto max-w-5xl px-4 pt-6 sm:px-6 space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {options.map((o) => (
            <Link
              key={`${o.year}-${o.month}`}
              href={`/donasi/laporan/${campaignId}?year=${o.year}&month=${o.month}`}
      ...
      <DetailReportView
        report={report}
        backHref="/donasi"
        backLabel="Kembali ke Donasi"
        downloadKind="donasi"
        downloadId={campaignId}
