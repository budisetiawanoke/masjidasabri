...
export const metadata: Metadata = { title: "Laporan Infaq & Sadaqah" };
export const dynamic = "force-dynamic";

export default async function InfaqLaporanDetailPage({
  params,
  searchParams,
}: {
  ...
  const report = await loadDetailReport("infaq", category, year, month);
  if (!report) notFound();
  ...
  return (
    <>
      <div className="mx-auto max-w-5xl px-4 pt-6 sm:px-6 space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {options.map((o) => (
            <Link
              key={`${o.year}-${o.month}`}
              href={`/infaq-sadaqah/laporan/${category}?year=${o.year}&month=${o.month}`}
      ...
      <DetailReportView
        report={report}
        backHref="/infaq-sadaqah"
        backLabel="Kembali ke Infaq & Sadaqah"
        downloadKind="infaq"
        downloadId={category}
